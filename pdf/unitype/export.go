...
// ParseFile parses the truetype font from pdffile given by path.
...
func (f *Font) GetMetrics() Metrics {
	q := Metrics{
		...
		XHeight:            int(f.hhea.ascender) * 1000 / int(f.head.unitsPerEm),
		ItalicAngle:        f.post.italicAngle.Float64(),
		UnderlinePosition:  int(f.post.underlinePosition) * 1000 / int(f.head.unitsPerEm),
